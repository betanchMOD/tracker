	...
	<BlockContainer>
		...
		<section class="relative items-center w-full py-12">
			...
			<div class="grid gap-6 mt-4 md:grid-cols-3">
				<NuxtLink v-for="project in posts" :key="project.id" :href="`/posts/${project.slug}`"
					...
					<div class="relative h-56 overflow-hidden rounded-card group">
						<NuxtImg v-if="project.image" :src="project.image as string"
						...
						<div
							...
							<div class="p-8">
								...
								<div v-if="project?.built_with" class="flex flex-wrap gap-2 mt-2">
									<UBadge v-for="(item, itemIdx) in project?.built_with" :key="itemIdx" size="md"
										color="black">
										{{ item }}
									</UBadge>
								</div>
